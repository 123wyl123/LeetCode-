...
public class FiftyFour {
    public int fourSumCount(int[] A, int[] B, int[] C, int[] D) {
        ...
        for (int i = 0; i <lengthA ; i++) {
            ...
            if (A[i]+B[lengthB-1]+C[lengthC-1]+D[lengthD-1]<0)
            {
             continue;
            }
            for (int j = 0; j <lengthB ; j++) {
                ...
                if (A[i]+B[j]+C[lengthC-1]+D[lengthD-1]<0)
                {
                    continue;
                }
                int left =0;
                ...
                while (left<lengthC&&right>=0)
                {
                    int mid = (left+right)/2;
                    if (C[left]+D[right]<-sum)
                    {
                        left++;
                    }else if (C[left]+D[right]>-sum)
                    {
                        right--;
                    }else
                    {   int temp=1;
                        int temp1=1;
                        while (left+1<lengthC&&C[left]==C[left+1])
                        {
                            left++;
                           temp++;
                        }
    ...
    public static void main(String[] args) {
        ...
        new FiftyFour().fourSumCount(a,b,c,d);
    }
}
